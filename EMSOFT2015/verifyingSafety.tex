...
\todo[inline]{ASIDE: Could we extract the timed automaton from CFG of real controller and compare it to modeled HCHA? If it is not an abstraction either refine HCHA or refine controller? Could be really interesting for ML applications. Let machine learn new controller. Extract CFG. See if CFG is consistent with desired HCHA behavior. Alternately can we use the APEX framework as some sort of oracle for ML applications.}
